<?php $title = 'Sửa sách: ' . e($book['title']) ?>
<?php $breadcrumbs = [
...
<div class="admin-form-page">
    <div class="admin-form-shell wide">
        <h1 class="admin-page-title" style="margin-bottom:1.5rem">Sửa sách: <?= e($book['title']) ?></h1>

        <form method="POST" action="<?= route('admin.books.update', ['book' => $book['id']]) ?>" enctype="multipart/form-data" class="admin-form-card">
        <?= csrf_field() ?>
        <?= method_field('PUT') ?>
        ...
        <div class="admin-form-grid cols-2" style="margin-bottom:1rem">
                ...
                <div class="admin-select-wrap">
                    <select name="category_id" required>
                        ...
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= $cat['id'] ?>" <?= $book['category_id'] == $cat['id'] ? 'selected' : '' ?>><?= e($cat['name']) ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
            </div>
            <div>
                ...
                <input type="number" name="published_year" value="<?= e($book['published_year'] ?? '') ?>"
        ...
        <div style="margin-bottom:1rem">
            ...
            <textarea name="description" rows="4" maxlength="5000"
                      placeholder="Nhập mô tả về cuốn sách..." class="admin-textarea"><?= e($book['description'] ?? '') ?></textarea>
        ...
        <div style="margin-bottom:1rem">
            ...
            <?php if (!empty($book['cover_image'])): ?>
                <div style="margin-bottom:0.75rem;display:flex;align-items:center;gap:1rem">
                    <img src="/uploads/<?= e($book['cover_image']) ?>" alt="Cover hiện tại"
                         style="width:5rem;height:7rem;object-fit:cover;border-radius:0.5rem;border:1px solid #e5e7eb">
                    <span style="font-size:0.75rem;color:#9ca3af">Ảnh bìa hiện tại. Chọn ảnh mới bên dưới để thay đổi.</span>
                </div>
            <?php endif ?>
